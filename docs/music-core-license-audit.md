...
本轮新增 `better-sqlite3` 11.10.0（MIT）用于账号与用户状态持久化；数据库运行时文件不进入发行产物。Wikimedia Commons 与 Internet Archive Provider 只调用官方 API；前者按文件来源页保留独立许可要求，后者仅在条目提供明确 Creative Commons 或 Public Domain 许可链接时开放下载，均不授予统一再分发权。Brave Search 集成使用官方远程 API，没有引入其软件代码或新 npm 依赖；索引结果只链接平台公开页面，不取得页面或音乐内容的再分发权。维护者在发布前仍需选择目标项目许可证，并确认演示音频及所有外部服务的内容条款；软件许可证不自动授予音乐内容的再分发权。
